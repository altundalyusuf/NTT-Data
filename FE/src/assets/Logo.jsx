export const LetterN = ()=>{
    return(
        <svg width="25" height="23" viewBox="0 0 26 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path id="n" d="M19.3765 17.952C19.2688 17.76 11.3029 3.936 10.3341 2.688C9.25765 1.056 7.96588 0 5.27471 0C2.90647 0 0 0.96 0 6.144V23.52H5.92059V9.024C5.92059 7.968 5.81294 6.432 5.81294 6.144C5.81294 5.952 5.81294 5.664 5.92059 5.568C6.13588 5.472 6.24353 5.664 6.35118 5.76C6.45882 5.856 13.8865 18.912 15.3935 21.024C16.47 22.56 17.8694 23.712 20.4529 23.712C22.8212 23.712 25.7276 22.752 25.7276 17.568V0.192H19.8071V14.688C19.8071 15.744 19.9147 17.28 19.9147 17.568C19.9147 17.76 19.9147 18.048 19.8071 18.144C19.6994 18.336 19.5918 18.144 19.3765 17.952Z" fill="#0059BC"/>
        </svg>
    )
}

export const LetterT = ()=>{
    return(
        <svg width="23" height="23" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path id="t" d="M0.881775 0.191986V5.18399H9.27825V23.52H15.1988V5.18399H23.5953V0.191986H0.881775Z" fill="#0059BC"/>
        </svg>
    )
}
export const LetterT1 = ()=>{
    return(
        <svg width="23" height="23" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path id="t_1_" d="M0.316467 0.191986V5.18399H8.71294V23.52H14.6335V5.18399H23.03V0.191986H0.316467Z" fill="#0059BC"/>
        </svg>

    )
}
export const LetterD = ()=>{
    return(
        <svg width="24" height="23" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path id="d" d="M15.9588 0.191986H0.134705V23.424H16.2818C22.6329 23.52 24.6782 20.64 24.6782 15.264V8.44799C24.6782 2.30399 21.9871 0.191986 15.9588 0.191986ZM18.65 15.456C18.65 17.664 17.7888 18.528 15.6359 18.528H6.05529V5.27999H15.6359C17.7888 5.27999 18.65 6.14399 18.65 8.35199V15.456Z" fill="#0059BC"/>
        </svg>
    )
}
export const LetterA = ()=>{
    return(
        <svg width="24" height="23" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path id="a" d="M7.8841 18.528C7.02292 18.528 5.94645 18.144 5.94645 16.128C5.94645 14.112 6.91527 13.728 7.8841 13.728H19.0794V18.432L7.8841 18.528ZM16.4959 0.191986H1.64056V5.18399H16.1729C18.3259 5.18399 19.187 6.04799 19.187 8.25599V9.02399H7.13056C2.50173 9.02399 0.24115 11.04 0.24115 15.744V16.8C0.24115 21.6 2.60939 23.52 7.34586 23.52H25V8.25599C24.8923 2.39999 22.847 0.191986 16.4959 0.191986Z" fill="#0059BC"/>
        </svg>
    )
}
export const LetterT2 = ()=>{
    return(
        <svg width="23" height="23" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path id="t_2_" d="M0.772949 0.191986V5.18399H9.16942V23.52H15.09V5.18399H23.4865V0.191986H0.772949Z" fill="#0059BC"/>
        </svg>
    )
}
export const LetterA1 = ()=>{
    return(
        <svg width="24" height="23" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path id="a_1_" d="M7.68939 18.528C6.82821 18.528 5.75174 18.144 5.75174 16.128C5.75174 14.112 6.72056 13.728 7.68939 13.728H18.8847V18.432L7.68939 18.528ZM16.3012 0.191986H1.44585V5.18399H15.9782C18.1311 5.18399 18.9923 6.04799 18.9923 8.25599C18.9923 8.25599 18.9923 8.92799 18.9923 9.02399H6.93585C2.30703 9.02399 0.0464478 11.04 0.0464478 15.744V16.8C0.0464478 21.6 2.41468 23.52 7.15115 23.52H24.8053V8.25599C24.8053 2.39999 22.5447 0.191986 16.3012 0.191986Z" fill="#0059BC"/>
        </svg>
    )
}







